...
function Barriers(height, width, opening, space, notifyDot) {
  ...
  this.animate = () => {
    this.couples.forEach((couple) => {
      ...
      const crossedMiddle =
        couple.getX() + displacement >= middle && couple.getX() < middle;
...
function Bird(gameHeight) {
  let flying = false;
  this.element = newElement("img", "unicornio");
  this.element.src = "assets/img/unicornio.png";
  this.getY = () => parseInt(this.element.style.bottom.split("px")[0]);
  this.setY = (y) => (this.element.style.bottom = `${y}px`);
  window.ontouchstart = (e) => (flying = true);
  window.ontouchend = (e) => (flying = false);
  this.animate = () => {
    const newY = this.getY() + (flying ? 2.7 : -3.5);
